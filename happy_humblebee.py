...
# Load the music file
pygame.mixer.music.load('fly_groove.mp3')
...
class Humblebee(pygame.sprite.Sprite):
    ...
    def get_rect(self):
        ...
        rect = self.rect.copy()
        rect.inflate_ip(-40, -40)  # reduce hitbox size
        return rect
...
while run:
    ...
    draw_text(str(score), font, white, int(screen_width / 2 - font.size(str(score))[0] / 2), 36)
    # draw the highscore at the bottom left
    draw_text(f'Highscore: {high_score}', font, white, 10, screen_height - 116)
